...
function TrendChart({ composed = false }: { composed?: boolean }) {
  const ChartRoot = composed ? ComposedChart : AreaChart;
  return <ResponsiveContainer width="100%" height="100%"><ChartRoot data={trend} margin={{ top: 34, right: 12, left: -22, bottom: 0 }}>
    ...
    {composed ? <><Bar isAnimationActive={false} dataKey="output" name="产量" fill="var(--chart-green)" radius={[3,3,0,0]} barSize={18}/><Line isAnimationActive={false} dataKey="load" name="负荷" stroke="var(--chart-cyan)" strokeWidth={3} dot={false}/></> : <><Area isAnimationActive={false} type="monotone" dataKey="output" name="实时产能" stroke="var(--chart-cyan)" strokeWidth={3} fill="url(#areaCyan)"/><Line isAnimationActive={false} type="monotone" dataKey="target" name="目标线" stroke="var(--chart-amber)" strokeDasharray="5 5" dot={false}/></>}
  </ChartRoot></ResponsiveContainer>;
}
