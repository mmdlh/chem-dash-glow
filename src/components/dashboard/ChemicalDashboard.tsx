...
function TrendChart({ composed = false }: { composed?: boolean }) {
  const C = composed ? ComposedChart : AreaChart;
  return <ResponsiveContainer width="100%" height="100%"><C data={trend} margin={{ top: 34, right: 12, left: -22, bottom: 0 }}>
    <defs><linearGradient id="areaCyan" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="var(--chart-cyan)" stopOpacity={0.55}/><stop offset="100%" stopColor="var(--chart-cyan)" stopOpacity={0}/></linearGradient></defs>
    <CartesianGrid stroke="var(--grid-line)" strokeDasharray="3 5" vertical={false}/><XAxis dataKey="t" tick={axis} axisLine={false} tickLine={false}/><YAxis tick={axis} axisLine={false} tickLine={false}/>
    <Tooltip content={<ChartTip />}/><Legend verticalAlign="top" align="right" height={28} iconType="circle" wrapperStyle={{ top: 4, fontSize: 11 }}/>
    {composed ? <><Bar dataKey="output" name="产量" fill="var(--chart-green)" radius={[3,3,0,0]} barSize={18}/><Line dataKey="load" name="负荷" stroke="var(--chart-cyan)" strokeWidth={3} dot={false}/></> : <><Area type="monotone" dataKey="output" name="实时产能" stroke="var(--chart-cyan)" strokeWidth={3} fill="url(#areaCyan)"/><Line type="monotone" dataKey="target" name="目标线" stroke="var(--chart-amber)" strokeDasharray="5 5" dot={false}/></>}
