...
function TrendChart({ composed = false }: { composed?: boolean }) {
  if (composed) return <ResponsiveContainer width="100%" height="100%"><ComposedChart data={trend} margin={{ top: 34, right: 12, left: -22, bottom: 0 }}><CartesianGrid stroke="var(--grid-line)" strokeDasharray="3 5" vertical={false}/><XAxis dataKey="t" tick={axis} axisLine={false} tickLine={false}/><YAxis tick={axis} axisLine={false} tickLine={false}/><Tooltip content={<ChartTip />}/><Legend verticalAlign="top" align="right" height={28} iconType="circle" wrapperStyle={{ top: 4, fontSize: 11 }}/><Bar isAnimationActive={false} dataKey="output" name="产量" fill="var(--chart-green)" radius={[3,3,0,0]} barSize={18}/><Line isAnimationActive={false} type="monotone" dataKey="load" name="负荷" stroke="var(--chart-cyan)" strokeWidth={3} dot={false}/></ComposedChart></ResponsiveContainer>;
  return <ResponsiveContainer width="100%" height="100%"><LineChart data={trend} margin={{ top: 34, right: 12, left: -22, bottom: 0 }}><CartesianGrid stroke="var(--grid-line)" strokeDasharray="3 5" vertical={false}/><XAxis dataKey="t" tick={axis} axisLine={false} tickLine={false}/><YAxis tick={axis} axisLine={false} tickLine={false}/><Tooltip content={<ChartTip />}/><Legend verticalAlign="top" align="right" height={28} iconType="circle" wrapperStyle={{ top: 4, fontSize: 11 }}/><Line isAnimationActive={false} type="monotone" dataKey="output" name="实时产能" stroke="var(--chart-cyan)" strokeWidth={3} dot={{ r: 3, fill: "var(--chart-cyan)" }}/><Line isAnimationActive={false} type="monotone" dataKey="target" name="目标线" stroke="var(--chart-amber)" strokeWidth={2} strokeDasharray="5 5" dot={false}/></LineChart></ResponsiveContainer>;
}
